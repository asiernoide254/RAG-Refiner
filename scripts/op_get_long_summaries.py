...
def modify_html_to_txt(input_html):
    '''
    Extracts the long summary of a One Piece chapter.

    Args:
        input_html (str): The full HTML document for the chapter's Fandom page.

    Returns:
        long_summary (str): String value that contains every paragraph from the long summary of the chapter (one paragraph per line).
    '''
    ...
    long_summary_h2 = soup.find('div', class_='mw-parser-output').find_all('h2')

    for h2_element in long_summary_h2:
        # Check if the matching h2 element is found
        if 'Long Summary' in h2_element.get_text():
            # Find all paragraphs after the matching h2 element
            for sibling in h2_element.find_next_siblings():
                if sibling.name == 'p':
                    # Add the text content of each paragraph to long_summary
                    long_summary += sibling.get_text()
...
def extract_summaries(output_folder, last_chapter):
    ...
    # Store every paragraph from the long summary of every chapter in a list
    paragraph_list = []
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Get long summaries from the One Piece wiki by Fandom")
